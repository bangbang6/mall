...
<template>
  <div>
    ...
    <nav-bread><span>order</span></nav-bread>
...
export default {
  ...
  methods: {
      ...
      payMent(){
          ...
          axios.post('/users/addDingdan',{addressId:addressId,totleNum:this.orderTotal}).then(res=>{
              var res = res.data
              if(res.status == '0'){
                  console.log('add dingdan suc')
                  this.orderId = res.result.orderId
                  this.$router.push({path:'/success',query:{orderId:this.orderId}})
              }
          })
      }
  }
